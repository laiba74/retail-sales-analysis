
## Excel Data Cleaning

### 1. Raw Dataset Issue
The original dataset had date formatting issues in the Order Date and Ship Date columns.

![Raw Dataset Issue](raw_dataset.png)

### 2. Missing Values Check
Checked the dataset for blank cells in important columns using Excel's Go To Special feature.

![Missing Values Check](missing_values_check.png)

### 3. Date Conversion Step
Used Excel's Text to Columns feature to convert date fields into a proper date format.

![Date Conversion Step](date_conversion_step.png)

### 4. Final Cleaned Dataset
Created cleaned date fields and added new columns such as Order Year and Order Month for analysis.

![Final Cleaned Dataset](cleaned_dataset.png)
